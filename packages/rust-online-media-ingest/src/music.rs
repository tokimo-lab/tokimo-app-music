    ...
    pub fn merge_with(&self, fallback: &Self) -> Self {
        Self {
            ...
            album_artist: self
                .album_artist
                .clone()
                .or(fallback.album_artist.clone()),
            ...
            release_date: self
                .release_date
                .clone()
                .or(fallback.release_date.clone()),
        }
    }
...
pub fn detect_ytdlp_content_type(raw_metadata: Option<&Value>) -> String {
    let music_metadata = extract_music_metadata(raw_metadata);
    if music_metadata.has_signal() || has_music_category(raw_metadata) || has_music_url_hint(raw_metadata)
    {
        "music".into()
    } else {
        "online_video".into()
    ...
    raw_metadata
        ...
        .map(|categories| {
            categories.iter().any(|entry| {
                entry
                    .as_str()
                    .map(|text| text.eq_ignore_ascii_case("music"))
                    .unwrap_or(false)
            })
        })
        .unwrap_or(false)
...
fn has_music_url_hint(raw_metadata: Option<&Value>) -> bool {
    first_string(raw_metadata.unwrap_or(&Value::Null), &["webpage_url"])
        .map(|url| url.contains("music.youtube.com"))
        .unwrap_or(false)
}
